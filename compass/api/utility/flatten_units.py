...
class UnitRecord(NamedTuple):
    name: str
    parent: int
    children: Union[tuple[tuple[int, str], ...], None]
    sections: Union[tuple[tuple[int, str], ...], None]


def _flatten_hierarchy(d: Union[schema.UnitData, schema.DescendantData], parent_id: int) -> Iterator[tuple[int, UnitRecord]]:
    unit_id = d.unit_id
    sections = tuple((section.unit_id, section.name) for section in d.sections) or None
    children = tuple((child.unit_id, child.name) for child in d.child) if d.child else None
...
def load_hierarchy_map(path: Path = FLAT_PATH) -> dict[int, UnitRecord]:
    ...
    for unit_id, unit_data in json_hierarchy.items():
        ...
        if children:
            children = tuple(tuple(child) for child in children)
        flat_hierarchy[unit_id] = UnitRecord(name, parent_id, children, sections)
    return flat_hierarchy
