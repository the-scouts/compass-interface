...
@contextlib.contextmanager
def get_cached_json(filename: Path, /, *, expected_type: type = type(None)):
    try:
        if Settings.cache_to_file is True:
            ...
            if json_data:
                if type(None) != expected_type and isinstance(json_data, expected_type):
                    yield json_data
                else:
                    yield json_data
