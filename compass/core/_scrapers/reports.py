...
def get_report_page(client: counting_session.CountingSession, run_report_url: str) -> bytes:
    ...
    report_page = client.get(f"{Settings.base_url}/{run_report_url}")

    return report_page.content


def update_form_data(client: counting_session.CountingSession, report_page: bytes, run_report: str) -> None:
    ...
    report = client.post(run_report, data=form_data, headers={"X-MicrosoftAjax": "Delta=true"})
...
def report_keep_alive(client: counting_session.CountingSession, report_page: str) -> str:
...
def download_report_streaming(client: counting_session.CountingSession, url: str, params: dict[str, str], filename: str) -> None:
...
def download_report_normal(client: counting_session.CountingSession, url: str, params: dict[str, str], filename: str) -> bytes:
    start = time.time()
